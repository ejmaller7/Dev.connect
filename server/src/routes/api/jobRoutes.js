...
router.get('/remote-jobs', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to fetch remote job data');
    }
    ...
    const developerJobs = filteredJobs
      .filter(job =>
        job.position && (
...
router.get('/onsite-jobs', async (req, res) => {
  ...
  try {
    const response = await fetch(apiUrl, {
      method: 'GET',
      headers: {
        'Authorization': `Token 9868b5ad5619fe6d0d5e64c103bfb583ccc1d1de`,
      }
    });

    if (!response.ok) {
      throw new Error('Failed to fetch on-site job data');
    }
    ...
    // Filter the jobs based on the role (developer, engineer, software)
...
// Fetch Both Remote and On-Site Jobs
